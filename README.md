...
flowchart LR
    ...
    subgraph App1["Beat Detector + ComfyUI Bridge"]
        WASM[Essentia.js WASM<br/>Lazy Init Singleton]
        Beats[RhythmExtractor2013<br/>Beat Detection]
        Onsets[OnsetRate<br/>Onset Detection]
        Loud[Loudness<br/>Energy Analysis]
        Export[Extended CSV Export]
        WASM --> Beats --> Export
        WASM --> Onsets --> Export
        WASM --> Loud --> Export
    end
    ...
    subgraph Resolve["DaVinci Resolve"]
        TMarkers[Timeline Markers<br/>beats, loudness]
        CMarkers[Clip Markers<br/>onsets]
    end
