...
if __name__ == '__main__':
    main()
`;
        const baseName = data.audioPath ? path_1.default.basename(data.audioPath, path_1.default.extname(data.audioPath)) : 'Untitled';
...
electron_1.ipcMain.handle('list-resolve-scripts', async () => {
    try {
        ...
        const files = fs_1.default.readdirSync(resolveScriptsDir);
        const scriptFiles = files
            .filter(f => f.endsWith('.py'))
            .map(f => {
            const fullPath = path_1.default.join(resolveScriptsDir, f);
